...
- **总任务数**: 76
- **已完成**: 17 (Phase 1: 11 tasks, Phase 2: 6 tasks)
- **进行中**: 0
- **待开始**: 59
- **预计工时**: 80 小时（10 个工作日）
- **已用工时**: 约 20 小时（Phase 1: 14h, Phase 2: 6h）

### 阶段进度
- ✅ **Phase 1**: 设置页面基础框架 (11/11 tasks, 100%)
- ✅ **Phase 2**: Content Script 联动 (6/6 tasks, 100%)
...
- [ ] **TASK-037**: 添加"重置为默认"按钮到设置页面
  - 在 `src/options/index.html` 底部添加按钮
  - 应用样式 `.reset-button`（红色警告样式）
  - 工时: 0.5 小时
  - 负责人: 开发工程师

- [ ] **TASK-038**: 实现 `resetToDefault()` 函数
  - 在 `src/options/toolActions.ts` 中添加函数
  - 显示确认对话框（使用原生 `confirm()` 或自定义对话框组件）
  - 用户确认后，调用 `savePreferences(DEFAULT_PREFERENCES)`
  - 刷新工具列表 UI（调用 `ToolList.render()`）
  - 工时: 2 小时
  - 负责人: 开发工程师

- [ ] **TASK-039**: 为重置按钮添加点击事件监听器
  - 在 `src/options/index.ts` 中添加事件监听器
  - 点击时调用 `resetToDefault()`
  - 工时: 0.5 小时
  - 负责人: 开发工程师
...
- [ ] **TASK-040**: 实现配置数据校验逻辑
  - 在 `src/lib/storage.ts` 的 `loadPreferences()` 中添加校验
  - 检查 toolOrder 是否包含重复 ID（使用 Set 去重）
  - 检查 toolOrder 是否包含无效 ID（不在 TOOLS 数组中）
  - 检查 enabledTools 是否包含重复或无效 ID
  - 如果检测到异常，返回 DEFAULT_PREFERENCES
  - 工时: 2 小时
  - 负责人: 开发工程师

- [ ] **TASK-041**: 显示配置数据异常的警告提示
  - 在设置页面顶部添加警告横幅元素（`.warning-banner`）
  - 检测到数据异常时显示："配置数据异常，已恢复为默认设置"
  - 提供关闭按钮（点击后隐藏横幅）
  - 工时: 1.5 小时
  - 负责人: 开发工程师

- [ ] **TASK-042**: 处理所有工具被禁用的情况（防御性编程）
  - 注意：由于 FR-011 约束，此情况理论上不可达（toggleToolEnabled 会阻止禁用最后一个工具）
  - 在 `ToolDropdown` 类中检查 enabledTools 是否为空（防御性代码）
  - 如果为空，显示 "无可用工具" 提示（灰色文本，居中显示）
  - 备选方案：完全隐藏侧边栏按钮（在 `SidebarButton` 类中添加逻辑）
  - 工时: 0.5 小时
  - 负责人: 开发工程师

### 测试与验证

- [ ] **TASK-043**: 编写单元测试：`resetToDefault()`
  - 测试重置逻辑（配置恢复为默认）
  - Mock `confirm()` 返回 true（用户确认）
  - Mock `confirm()` 返回 false（用户取消）
  - 工时: 0.5 小时
  - 负责人: 开发工程师

- [ ] **TASK-044**: 编写单元测试：配置数据校验
  - 测试正常配置（通过校验）
  - 测试包含重复 ID 的 toolOrder（自动去重或恢复默认）
  - 测试包含无效 ID 的 toolOrder（过滤无效 ID 或恢复默认）
  - 测试空的 toolOrder（使用默认顺序）
  - 工时: 1 小时
  - 负责人: 开发工程师

- [ ] **TASK-045**: 手动测试重置功能与边界情况
  - 自定义配置后点击重置，验证配置恢复为默认
  - 手动修改 chrome.storage.sync 数据（模拟数据损坏），验证自动恢复并显示警告
  - 禁用所有工具，验证工具菜单显示 "无可用工具"
  - 工时: 1 小时
  - 负责人: 开发工程师

- [ ] **TASK-046**: 阶段 4 代码审查与提交
  - 提交代码，代码审查，合并
  - 工时: 1 小时
  - 负责人: 开发工程师 + 审查员
...
- [ ] **TASK-047**: 编写单元测试：`loadToolConfiguration()`
  - 测试加载默认配置（首次安装）
  - 测试加载自定义配置（已有用户数据）
  - 测试加载损坏的配置（数据校验失败）
  - 工时: 1 小时
...
- [ ] **TASK-048A**: 验证存储数据大小约束（NFR-004）
  - 创建完整的工具配置数据（9 个工具，自定义顺序）
  - 序列化为 JSON 并测量字节大小
  - 验证总大小 ≤5KB（包含 toolOrder, enabledTools, meta 字段）
  - 如果超出限制，优化数据结构（如移除冗余字段）
  - 工时: 0.5 小时
  - 负责人: 开发工程师
  - 修复失败的测试
  - 工时: 1.5 小时
...
- [ ] **TASK-049**: 编写 E2E 测试：启用/禁用切换流程
  - 使用 Playwright 或 Puppeteer
  - 打开设置页面 → 禁用工具 → 打开 GitHub 页面 → 验证工具不显示
  - 重新启用工具 → 验证工具显示
  - 工时: 2 小时
  - 负责人: 测试工程师

- [ ] **TASK-050**: 编写 E2E 测试：重置为默认流程
  - 自定义配置 → 点击重置按钮 → 确认 → 验证配置恢复默认
  - 工时: 1 小时
  - 负责人: 测试工程师

- [ ] **TASK-051**: 编写 E2E 测试：跨实例同步
  - 在 Chrome 实例 A 修改配置
  - 等待 5 秒
  - 在 Chrome 实例 B 验证配置已同步
  - 工时: 1.5 小时
  - 负责人: 测试工程师

### 性能测试

- [ ] **TASK-052**: 性能测试：chrome.storage.sync 保存延迟
  - 使用 Performance API 测量保存操作的延迟
  - 验证延迟 ≤500ms（P95）
  - 工时: 1 小时
  - 负责人: 测试工程师

- [ ] **TASK-053**: 性能测试：设置页面加载时间
  - 测量从打开页面到工具列表渲染完成的时间
  - 验证加载时间 ≤200ms（P95）
  - 工时: 1 小时
  - 负责人: 测试工程师
...
- [ ] **TASK-054**: 优化拖拽动画效果
  - 调整 SortableJS 动画时长（150ms → 200ms）
  - 调整半透明副本的透明度（opacity: 0.5）
  - 添加拖拽结束时的缓动效果（ease-out）
  - 工时: 1.5 小时
  - 负责人: 开发工程师

- [ ] **TASK-055**: 优化错误提示样式
  - 警告横幅使用黄色背景 + 警告图标
  - 确认对话框使用自定义样式（替代原生 `confirm()`，可选）
  - "无可用工具" 提示使用灰色文本 + 提示图标
  - 工时: 1.5 小时
  - 负责人: 开发工程师
...
- [ ] **TASK-056**: 添加 ARIA 属性
  - 为拖拽手柄添加 `aria-label="拖拽以调整顺序"`
  - 为开关按钮添加 `role="switch"` 和 `aria-checked`
  - 为"重置为默认"按钮添加 `aria-label="重置为默认配置"`
  - 工时: 1 小时
  - 负责人: 开发工程师

- [ ] **TASK-057**: 测试键盘导航
  - 验证可使用 Tab 键在开关按钮间切换
  - 验证可使用 Enter/Space 键切换开关状态
  - 验证可使用 Enter 键触发重置按钮
  - 工时: 0.5 小时
  - 负责人: 测试工程师
...
- [ ] **TASK-059**: 手动测试所有功能（完整流程）
  - 按照 MANUAL_TESTING.md 检查清单执行测试
  - 在 Windows、macOS、Chrome OS 上测试
  - 工时: 2 小时
  - 负责人: 开发工程师 + 测试工程师

- [ ] **TASK-060**: 阶段 5 代码审查与提交
  - 提交代码，代码审查，合并
  - 工时: 1 小时
  - 负责人: 开发工程师 + 审查员
